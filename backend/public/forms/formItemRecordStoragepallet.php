          ...
          <table>
            <form class="" action="<?php echo(static::$ResourceName . '?accessToken=' . SessionManager::GetAccessToken());?>" method="post">
              ...
              <tr>
                ...
                <td>
                <?php
                  //Grab the correct binID for this record
                  $binAssigned = DataProvider::GetBinForPallet($this->ID);
                  $binID =  (empty($binAssigned)) ? 0 : $binAssigned[0]['binid'];
                  //Show the bins in a select
                  $bins = new Storagebin(0);
                  $rowData = SessionManager::GetAccessibleRecordIDs("Storagebin");
                  $indexFieldName = Util::array_key_first($rowData[0]);
                  echo $bins->ShowSelectDropDownComponent($binID, $rowData, 'binID',null, null, $indexFieldName);
                ?></td>
              </tr>
